...
class TestMain:
    ...
    def test_01_create_valid_snap_msg(self, mock_uuid):
        response = self.client.post("/snaps/", json={"message": "Hello"}, )
    ...
    def test_03_create_empty_snap_msg(self):
        response = self.client.post("/snaps/", json={"message": ""}, )
    ...
    def test_04_create_long_snap_msg(self):
        response = self.client.post("/snaps/", json={"message": "a" * 281}, )
        assert response.status_code == Status.http_400_bad_request()
        assert response.json() == {
            "type": "about:blank",
    ...
    def test_05_get_snap_message_by_id(self):
        response = self.client.get("/snaps/12345678-1234-5678-1234-567812345678")
        assert response.status_code == Status.http_200_ok()
    ...
    def test_06_get_snap_message_by_invalid_id(self):
        response = self.client.get("/snaps/12345678-1234-5678-1234-567812345679")
    ...
    def test_07_delete_snap_message_by_id(self):
        response = self.client.delete("/snaps/12345678-1234-5678-1234-567812345678")
        assert response.status_code == Status.http_204_no_content()

    def test_08_delete_snap_message_by_invalid_id(self):
        response = self.client.delete("/snaps/12345678-1234-5678-1234-567812345679")
        assert response.status_code == Status.http_404_not_found()
        assert response.json() == {
            "type": "about:blank",
            "title": "Snap not found",
